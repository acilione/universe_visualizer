...
export function createConstellationView({catalog,figure,mode,observer,renderer}) {
  ...
  const positionNote=mode==='earth'
    ? `Cielo per ${observer.latitude.toFixed(2)}°, ${observer.longitude.toFixed(2)}° · ${new Date(observer.dateIso).toLocaleString('it-IT')}. ${visibleStarCount}/${members.length} stelle della figura sopra l’orizzonte. Luminosità amplificata; cielo diurno non simulato.`
